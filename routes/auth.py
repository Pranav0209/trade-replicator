...
async def login(user_id: str):
    ...
    Usage: http://127.0.0.1:8000/auth/login?user_id=CAY950
    """
    # Get user credentials from storage
    user = await db.accounts.find_one({"account_id": user_id})
    if not user:
        raise HTTPException(404, f"User {user_id} not found. Register with /auth/register first.")
    ...
    try:
        ...
        login_url = kite.login_url() + f"&state={user_id}"
...
@router.get("/callback")
async def callback(request_token: str, user_id: str = None, state: str = None):
    """
    Callback from Zerodha. Exchanges request_token for access_token.
    If user_id/state is missing, tries to match token against all registered accounts.
    """
    effective_user_id = user_id or state
    ...
    if effective_user_id:
        ...
        user = await db.accounts.find_one({"account_id": effective_user_id})
        if user:
            try:
                kite = KiteConnect(api_key=user["api_key"])
                ...
                print(f"Direct auth failed for {effective_user_id}: {e}")
    else:
        # 2. Fallback: Try all potential accounts
        print("Missing state/user_id in callback, searching for matching account...")
        ...
        for user in all_accounts:
            try:
                ...
                access_data = kite.generate_session(request_token, api_secret=user["api_secret"])
                found_user = user
                effective_user_id = user["account_id"]
                print(f"Matched request_token to user: {effective_user_id}")
                break
            except Exception as e:
    ...
    try:    
        ...
        try:
            ...
            print(f"Fetched live capital for {effective_user_id}: {live_capital} (Op: {opening_balance} + Col: {collateral} - Used: {used_margin})")
        except Exception as e:
            print(f"Failed to fetch margins for {effective_user_id}: {e}")
            live_capital = found_user.get("capital", 0)

        # Store access token and live capital
        await db.accounts.update_one(
            {"account_id": effective_user_id},
            {
                "$set": {
        ...
        return {
            "status": "success",
            "message": f"User {effective_user_id} authenticated successfully!",
...
async def register(user_id: str, api_key: str, api_secret: str):
    """
    Pre-register a user with their Zerodha API credentials.
    Usage: POST /auth/register?user_id=CAY950&api_key=xxxxx&api_secret=yyyyy
    """
    # Check if already registered
    existing = await db.accounts.find_one({"account_id": user_id})
    if existing:
    ...
    return {
        "status": "ok",
        "user_id": user_id,
        "next_step": f"Visit: http://127.0.0.1:8000/auth/login?user_id={user_id}"
    }
